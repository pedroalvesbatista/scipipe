...
func TestSetPathStatic(t *testing.T) {
	...
	mockTask := NewSciTask(wf, "echo_foo_task", "", nil, nil, nil, nil, "", p.ExecMode, 1)
...
func TestSetPathExtend(t *testing.T) {
	...
	mockTask := NewSciTask(wf, "echo_foo_task", "", map[string]*IP{"foo": NewIP("foo.txt")}, nil, nil, nil, "", p.ExecMode, 1)
